...
;**********************************************************************
;
;   Ce programme permet de traduires les scan-codes du jeux 3 d'un
;   clavier PS/2 vers les scan-codes Atari IKBD et de gerer une souris
;   PS/2 standard ou Intellimouse a roulette 3 boutons et double roulette
;   5 boutons. Les boutons 4&5 sont affectes a des scan-codes.
;
...
;       RC4: Tir
;
;   La commande d'un ventilateur est egalement possible :
...
;            En supposant que la CTN est utilisee dans sa zone "lineaire",
;            Temp = (IDX_T0_AN0 - Val_AN0) * IDX_SCALETEMP / 64 avec :
;            Temp = temperature en degres C
;            IDX_T0_AN0 = valeur AN0 pour 0 degres C en EEPROM
;            IDX_SCALETEMP = facteur d'echelle en EEPROM
;            Val_AN0 = valeur sur AN0 soit 0 pour 0V et 255 pour 4,3V soit :
;             Val_AN0 = (256 * RCTN)/(10E3+RCTN)
;             En mesurant 11200 ohms a 20 degres C et 5450 ohms a 40 degres C,
;             soit Val_AN0 = 135 a 20 degres C et Val_AN0 = 90 a 40 degres C
;             on peut en deduire que :
;             IDX_SCALETEMP = (64 * T(40)-T(20)) / (Val_AN0(20) - Val_AN0(40))
;                           = 28 pour cette resistance CTN
;             IDX_T0_AN0 = ( (64 * T(40)) / IDX_SCALETEMP ) + Val_AN0(40)
;                        = 180 pour cette resistance CTN
;
...
;La tranmission des octets clavier      ASCII          binaires.
;La tranmission des octets souris       ASCII          binaires.
...
DEBUG_CODE                   EQU     0; si 0, PORTB4 & PORTB5 enleves, mode Atari seulement
SCROOL_LOCK_ERR              EQU     0; si 1, utilise pour afficher les erreurs parite souris PS/2
                                      ;       sur la led Scroll Lock du clavier
NON_BLOQUANT                 EQU     0; si 1, routines PS/2 non bloquantes (time-out)
...
TAMPON_EM                    EQU    0x28; buffer circulaire de 16 octets liaison serie (TAILLE_TAMPON_EM dans laurent.inc)
...
YEAR_BCD                     EQU    0x3A; annee, conserver cet ordre pour les 6 variables BCD
MONTH_BCD                    EQU    0x3B; mois
DAY_BCD                      EQU    0x3C; jour
HRS_BCD                      EQU    0x3D; heures
MIN_BCD                      EQU    0x3E; minutes
SEC_BCD                      EQU    0x3F; secondes
...
Temp                         EQU    0x49; lecture temperature CTN sur AN0
...
PARITY                       EQU    0x76; parite ecriture/lecture PS/2
TEMP2                        EQU    0x77
MyStatus                     EQU    0x78; flags
MyStatus2                    EQU    0x79; flags
MyStatus3                    EQU    0x7A; flags
TEMP3                        EQU    0x7B
TEMP4                        EQU    0x7C
BUTTON_ACTION                EQU    0x7D; mode button action IKBD
BUFFER_FLASH                 EQU    0x7E; buffer 2 octets

; page 2 en zone non remise a 0 au reset par Start_Flash
...
Days_In_Month
		...
		IF DEBUG_CODE
;-----------------------------------------------------------------------------
;   Chaine de caracteres bienvenue !
;-----------------------------------------------------------------------------
...
;------------------------------------------------------------------------
;   Implementation d'une table de correspondance pour convertir en BCD le
;   contenu du registre w et l'envoyer vers la laison serie.
;
...
TstAtariReceive
			...
			DISABLEPS2; bloque clavier et souris
			call SerialReceive; => Value, on teste les commandes clavier Atari
...
;-----------------------------------------------------------------------------
;               Gestion sous Timer 2 a 5 mS de l'horloge.
;-----------------------------------------------------------------------------

doTimer
		DISABLEPS2; bloque clavier et souris
		bcf PIR1,TMR2IF; acquitte timer 2
		btfss MyStatus2,RESET_KEYB
			goto TstResetKeyb
		call cmdResetKey
		bcf MyStatus2,RESET_KEYB; flag demande reset clavier remis a 0
		goto TstClock
TstResetKeyb
		btfss MyStatus2,RESET_MOUSE
			goto TstClock
		call cmdResetMouse
		bcf MyStatus2,RESET_MOUSE; flag demande reset souris remis a 0
		goto TstClock			
TstClock
		incf Counter_5MS,F
		movlw 200
		subwf Counter_5MS,W
		btfsc STATUS,Z
			call INC_Clock; increment horloge toutes les 5 mS
		btfsc Counter_5MS,0
			goto Main
...
;-----------------------------------------------------------------------------
		...
		btfss MyStatus,JOY_ENABLE; gestion par timer joysticks
			goto Main; pas de gestion des joysticks
		btfsc MyStatus2,JOY_MONITOR
			goto Joy_Monitoring; monitoring des joysticks	
		btfsc MyStatus2,JOY_EVENT
			goto Not_100MS; evenements joysticks
		btfss MyStatus2,JOY_KEYS; envoi touches fleches
...
doAtariCommand
		bsf MyStatus,IKBD_ENABLE; transferts IKBD autorises	
		bcf MyStatus2,JOY_MONITOR; monitoring joysticks desactive
		;--------------------------------------------------------------
		;   Temperature
		movf Value,W
		...
		btfsc PORTC,MOTORON
			movlw 1
		call SerialTransmit_Host
		movlw Tab_Temp-EEProm+IDX_LEVELHTEMP
		call Lecture_EEProm
		call SerialTransmit_Host
		movlw Tab_Temp-EEProm+IDX_LEVELLTEMP
		call Lecture_EEProm
		call SerialTransmit_Host
		clrw
		call SerialTransmit_Host		
...
Not_Get_Temp
		...
		sublw IDX_SCALETEMP; soustraire l'offset MAX
		btfss STATUS,C; si carry =1 pas de problemes on passe
			goto Not_Prog_EEProm; ignore code > IDX_SCALETEMP
		movf Value,W
		addlw Tab_Temp-EEProm; pour pointer le debut des octets temperature dans la table
		goto Prog_EEProm
Not_Prog_Temp
		;--------------------------------------------------------------
		;   Programmation souris et clavier
		movf Value,W
		sublw IKBD_PROGMS; programmation clavier
		btfss STATUS,Z
			goto Not_Prog_Mouse
...
Not_Prog_Mouse
		movf Value,W
		sublw IKBD_PROGKB; programmation souris
		btfss STATUS,Z
			goto Not_ProgKB
		; Indiquer le mode programmation par allumages des trois LEDs
		movlw LEDS_ON
...
Not_Set_Mouse_Button_Action
		...
		btfss STATUS,Z
			goto Not_Rel_Mouse
		bcf MyStatus2,MOUSE_KEYS
		bcf MyStatus,MOUSE_ABS
		goto Loop_Mouse_Enable
Not_Rel_Mouse
		movf Value,W
...
Not_Abs_Mouse
		...
		bsf MyStatus2,MOUSE_KEYS
Loop_Mouse_Enable
		bsf MyStatus,MOUSE_ENABLE
		goto Main
Not_Mouse_KeyCode
		movf Value,W
		sublw IKBD_SET_MOUSE_THRESHOLD; non gere
		btfss STATUS,Z
			goto Not_Mouse_Threshold
		call SerialReceive; X seuil
		call SerialReceive; Y seuil
		goto Main
Not_Mouse_Threshold
		movf Value,W
...
Not_Pause
		...
		btfss STATUS,Z
			goto Not_Joy_On
		bcf MyStatus2,JOY_KEYS
		bsf MyStatus2,JOY_EVENT
		bsf MyStatus,JOY_ENABLE
		goto Main
Not_Joy_On
		movf Value,W
...
Not_Joy_Off
		...
		btfss MyStatus,JOY_ENABLE
			goto Main; joysticks inhibes
		btfsc MyStatus2,JOY_KEYS
...
Not_Interrog_Time
		...
		;   $00A0 a $00EF, $0120 a $016F, et de $01A0 a $01EF
...
Not_Execute
		...
		call Leds_Eiffel_On
		btfsc Status_Boot,POWERUP_KEYB
			call cmdDisableKey
		btfsc Status_Boot,POWERUP_MOUSE
			call cmdDisableMouse
		btfsc Status_Boot,POWERUP_KEYB
			bsf MyStatus2,RESET_KEYB; valide reset clavier dans traitement timer
		btfsc Status_Boot,POWERUP_MOUSE
			bsf MyStatus2,RESET_MOUSE; valide reset souris dans traitement timer
Init_MyStatus
		bsf MyStatus2,JOY_EVENT
		bsf MyStatus,MOUSE_ENABLE
		bsf MyStatus,JOY_ENABLE
		bsf MyStatus,IKBD_ENABLE	
		goto Main
Not_Reset
		;--------------------------------------------------------------
		;  Commandes de status
...
Mouse_Mode
		movlw HEADER_STATUS; header d'etat
		call SerialTransmit_Host
		btfsc MyStatus2,MOUSE_KEYS
			goto Mouse_Mode_Keys
		btfsc MyStatus,MOUSE_ABS
			goto Mouse_Mode_Abs
...
Loop_SerialTransmit_Status
		...
		IF DEBUG_CODE
;-----------------------------------------------------------------------------
;	Mode PC: Pas de translation, envoit direct des Scan-Codes en texte
...
doKeyboard
		...
		IF DEBUG_CODE
		btfss PORTB,JUMPER4; si jumper4 (+5V): Mode ATARI
			goto ModeKeyPC
		ENDIF
...
ModeKeyAtari
		...
		btfss STATUS,Z; si oui on passe
			goto Not_BreakCode
		bsf MyStatus2,BREAK_CODE; on doit traiter un break code (touche relachee)
                goto Main
Not_BreakCode
		btfsc MyStatus2,JOY_MONITOR
			goto Not_SendKey
		...
		bsf PORTB,LEDGREEN; eteint LED clavier
Not_SendKey
		bcf MyStatus2,BREAK_CODE; touche enfoncee par defaut
		goto Main
...
Normal_Mouse
		...
		;-------------------------------------------------------------
		;   Nouveaute: au lieu de simuler un appui droit et gauche, on genere un scan-code
...
ZZero
		...
		btfss MyStatus,MSWHEELPLUS;si bit WheelPlus a zero on transmet maintenant, il n'a pas de
...
;-----------------------------------------------------------------------------
; Reception de la commande BAT CLAVIER: On bascule le clavier en mode jeux 3
;-----------------------------------------------------------------------------
...
;-----------------------------------------------------------------------------
; Reception de la commande BAT SOURIS: On autorise la communication souris
;-----------------------------------------------------------------------------

OnMsBAT
		call cmdEnableMous
		call TryWheel; detecter si la souris est a roulette
		btfsc MyStatus,MSWHEEL; si c'est un souris a roulette
...
Read_Temp
		...
		movlw Tab_Temp-EEProm+IDX_T0_AN0
		call Lecture_EEProm
		movwf TEMP1
		movf ADRESH,W; 8 bits poids fort
		movwf Val_AN0; lecture CTN sur AN0
		subwf TEMP1,F; IDX_T0_AN0 - Val_AN0  
		btfss STATUS,C
			clrf TEMP1; Val_AN0 > IDX_T0_AN0
		movlw Tab_Temp-EEProm+IDX_SCALETEMP
		call Lecture_EEProm 
		movwf Counter
		clrf Temp; poids faible, TEMP2:Temp = (IDX_T0_AN0 - Val_AN0) * IDX_SCALETEMP
		clrf TEMP2; poids fort
Loop_Scale_Temp
			movf TEMP1,W
			addwf Temp,F
			clrw 
			...
			addwf TEMP2,F
			decfsz Counter,F
		goto Loop_Scale_Temp
                movlw 6; / 64
		movwf Counter
Loop_Scale2_Temp
		; temperature = TEMP2:Temp = (IDX_T0_AN0 - Val_AN0) * IDX_SCALETEMP / 64
			rrf TEMP2,F; poids fort
			rrf Temp,F; poids faible
			decfsz Counter,F
		goto Loop_Scale2_Temp
		movlw Tab_Temp-EEProm+IDX_LEVELHTEMP
		call Lecture_EEProm
		subwf Temp,W; Temp - IDX_LEVELHTEMP
		btfsc STATUS,C
			bsf PORTC,MOTORON; Temp >= IDX_LEVELHTEMP -> marche ventilateur	
		movlw Tab_Temp-EEProm+IDX_LEVELLTEMP
		call Lecture_EEProm
		subwf Temp,W; Temp - IDX_LEVELLTEMP
		btfss STATUS,C
			bcf PORTC,MOTORON; Temp < IDX_LEVELTEMP -> arret ventilateur	
		return
...
INC_Clock
		...
		btfsc ADCON0,GO_DONE
			goto Not_Read_Temp
		call Read_Temp; lecture temperature toutes les secondes
...
;	  scancode de la fleche clavier est alors envoye une 1ere fois suivant
;	  la direction choisie sur le joystick 0.
;	- Temps repetition TX/TY (X fois suivant VX/VY)
;	- Temps total VX/VY provoquant l'arret de l'envoi du scancode de la 
...
SendAtariMouse
		...
		btfsc MyStatus2,MOUSE_KEYS; envoi touches fleches
			goto Not_trame_Rel
		;-------------------------------------------------------------
...
Not_trame_Rel                      
		...
		btfss MyStatus2,MOUSE_KEYS; envoi touches fleches
			goto Not_Keys_Mouse
		movf X_MOV,W; deplacement relatif souris en X
...
Not_Clic_Mouse
		...
		btfsc STATUS,Z
			goto Roulettes
		; TRANSMISSION EFFECTIVE: Envoit du scan-code pour le bouton central
...
Repeat
			...
			btfsc STATUS,Z
				goto Buttons45
			; TRANSMISSION EFFECTIVE: Envoit du scan-code pour la molette
			call SerialTransmit_KeyUpDown; envoyer l'octet Molette
...
TryWheel
		...
		bcf MyStatus,MSWHEEL
		movf Value,W
		sublw MS_WHEEL; soustraire le code InteliMouse 3 boutons
		btfsc STATUS,Z
			bsf MyStatus,MSWHEEL; MS scrolling mouse
...
TryWheelPlus
		...
		bcf MyStatus,MSWHEELPLUS
		movf Value,W
		sublw MS_WHEELPLUS; soustraire le code InteliMouse 5 boutons
		btfsc STATUS,Z
			bsf MyStatus,MSWHEELPLUS; MS Intellimouse
		; JUST for test: si affichage BCD afficher le code ID de la souris
		IF DEBUG_CODE
		movf Value,W
		btfss PORTB,JUMPER5; si mode debug
...
;	Scan-codes type 3 du clavier PS/2.
;
;	Make code: 	<Octet touche>
;	Break code:	<$F0><Octet touche>
;
...
SendAtariKeyboard
		...
		iorlw 0; tester si translation a donne 0
		btfsc STATUS,Z; Z=1 si w contient 0
			return; pas d'affectation
		iorlw 0x80; relachement (BreakCode Bit7=1)
		btfsc MyStatus2,BREAK_CODE
			goto SerialTransmit_Host; break code
		andlw 0x7F; touche enfoncee
		call SerialTransmit_Host
...
; Sortie: Rien
;
; Global: TEMP2, TEMP3, TEMP4, TEMP5,
...
;-----------------------------------------------------------------------------
;   Envoit la commande Set Typematic Rate au clavier, pour fixer la 
...
;-----------------------------------------------------------------------------
;   Passage en jeux de scancode 3 clavier
;
;   Entree: Rien
;   Sortie: W = detruit
;
...
cmdScanSet3
		movlw SET_SCAN_CODE_SET; envoyer la commande de selection JEU 3
		call KPSCmd
		movlw SCANCODESET3
		goto KPSCmd

;-----------------------------------------------------------------------------
;   Envoit de la commande reset au CLAVIER
;
;   Entree: Rien
;   Sortie: W = detruit
;
...
;-----------------------------------------------------------------------------
;   Envoit de la commande resend au CLAVIER
;
;   Entree: Rien
;   Sortie: W = detruit
;
...
;-----------------------------------------------------------------------------
;   Envoit de la commande enable au CLAVIER
;
;   Entree: Rien
;   Sortie: W = detruit
;
...
;-----------------------------------------------------------------------------
;   Envoit de la commande disable au CLAVIER
;
;   Entree: Rien
;   Sortie: W = detruit
;
...
;-----------------------------------------------------------------------------
;   Envoit de la commande reset a la SOURIS
;
;   Entree: Rien
;   Sortie: W = detruit
;
...
;-----------------------------------------------------------------------------
;   Envoit de la commande resend a la SOURIS
;
;   Entree: Rien
;   Sortie: W = detruit
;
...
;-----------------------------------------------------------------------------
;   Envoit de la commande enable a la SOURIS
;
;   Entree: Rien
;   Sortie: W = detruit
;
...
;-----------------------------------------------------------------------------
;   Envoit de la commande disable a la SOURIS
;
;   Entree: Rien
;   Sortie: W = detruit
;
...
Leds_Eiffel_Off
		...
		ORG   900H; zone de 0x800 a 0x8FF pour le programme de flashage
...
;------------------------------------------------------------------------
; Envoit le contenu de W en 'BCD' sur la liaison serie.
; C'est a dire: W = 0x41 => Emission de l'ASCII '4' puis ASCII '1'
;
; Entree: W = donnee binaire
; Sortie: Rien
;
; Global: TEMP2, TEMP3 sont utilises
;------------------------------------------------------------------------
		IF DEBUG_CODE
SendBCD
		...
		movf TEMP3,W
                ; First part: process high nibble
		andlw 0xF0; Commencer par le MSB
		movwf TEMP2; transfere pour decalage 4 bits
		rrf TEMP2,F; logical shift right in F
		rrf TEMP2,F; logical shift right in F
		rrf TEMP2,F; logical shift right in F
		rrf TEMP2,W; logical shift right F put in W
		andlw 0x0F; clear upper nibble...
		bcf PCLATH,3; page 0 ou 2
		call LookUp; chercher le code ASCII
		bsf PCLATH,3; page 1 ou 3 (0x800 - 0xFFF)
		call SerialTransmit
                ; Second part: process low nibble
		movf TEMP3,W; reprendre la donnee
		andlw 0x0F; et traiter le LSB
		call LookUp; chercher le code ASCII
		goto SerialTransmit
...
SerialTransmit
		...
		movf TEMP4,W
		movwf TXREG; transmit byte
		bcf MyStatus2,DATATOSEND; flag rien a envoyer dans la boucle principale Main
		return
TxReg_Full
		movf STATUS,W
		...
		bcf STATUS,IRP; page 0
		incf FIN_INDEX_EM,W; incremente index buffer cirulaire dnnees a envoyer
		movwf FSR; pointeur index
		movlw TAILLE_TAMPON_EM
		subwf FSR,W
		btfsc STATUS,C
		...
		bsf MyStatus2,DATATOSEND; flag donnees a envoyer dans la boucle prinipale Main
End_SerialTransmit
		movf TEMP5,W
...
;-----------------------------------------------------------------------------
;               Division Counter3:Counter2 /  TEMP3 
...
;-----------------------------------------------------------------------------
;               Initialisation conversion BCD horloge 
;-----------------------------------------------------------------------------
...
; YEAR_BCD, MONTH_BCD, DAY_BCD, HRS_BCD, MIN_BCD, et SEC_BCD  
; => YEAR, MONTH, DAY, HRS, MIN, et SEC  
;
; Global: TEMP1, Counter1, Counter2, Counter3 sont utilises
;------------------------------------------------------------------------

Conv_Inv_BCD_Time
		call Init_Conv_BCD_Time
Loop_BCD_Time
			bcf STATUS,IRP; page 0
			movf Counter3,W
			movwf FSR
...
;------------------------------------------------------------------------
; Conversion binaire en BCD de l'horloge
; YEAR, MONTH, DAY, HRS, MIN, et SEC  
; => YEAR_BCD, MONTH_BCD, DAY_BCD, HRS_BCD, MIN_BCD, et SEC_BCD  
;
; Global: TEMP1, TEMP2, Counter1, Counter2, Counter3 sont utilises
;------------------------------------------------------------------------
...
Init
		...
		clrf MyStatus; mettre a zero mots d'etat
		clrf MyStatus2
		clrf MyStatus3
		comf PORTA,W
		movwf JOY0
		...
		movwf JOY1; lecture joystick 1
		clrf Val_AN0; lecture CTN sur AN0
		clrf Temp; lecture temperature CTN sur AN0
...
Ecriture_Flash	
		...
		WRITE_FLASH Counter,Counter2,BUFFER_FLASH; ecriture 2 octet => programme mauvais (0x3FFF)	
		clrf Value; checkum
...
Loop_Flash_2
				...
				addwf Value,F
				WRITE_FLASH Counter,Counter2,BUFFER_FLASH; ecriture 2 octets
				btfss Counter2,2
					bcf PORTB,LEDGREEN; allume les deux LEDs
				btfss Counter2,2
					bcf PORTB,LEDYELLOW
...
Loop_Flash_3
		...
		bsf Counter,4; pages 2-3 (0x1000 - 0x1FFF)
Loop_Flash_4
				READ_FLASH Counter,Counter2,BUFFER_FLASH; lecture 2 octets
			...
			btfss Counter,5
		goto Loop_Flash_4
		movf PARITY,W; lecture checksum
		subwf Value,W; checksum
		btfss STATUS,Z
			goto Fin_Prog_Flag; checksum mauvais
		movlw 0xFF
		...
		clrf BUFFER_FLASH
		clrf BUFFER_FLASH+1
		WRITE_FLASH Counter,Counter2,BUFFER_FLASH; ecriture 2 octets => programme OK (0)
Fin_Prog_Flag
		clrf PCLATH
		goto Reset_Prog
...
;-----------------------------------------------------------------------------
;               Zone EEPROM de stcokage des Scan-Codes Souris
...
Tab_Scan_Codes
		...
		DE 0x21; offset + 0x2B  jamais utilise
		...
		DE 0x0A; offset + 0x46  jamais utilise
		...
		DE 0x28; offset + 0x52 <%><ù>
		DE 0x2B; offset + 0x53 <µ><*> touche sur COMPAQ
		DE 0x1A; offset + 0x54 <¨><^>
		DE 0x0D; offset + 0x55 <+><=>
		...
		DE 0x1B; offset + 0x5B <£><$>
		DE 0x2B; offset + 0x5C <µ><*> touche sur SOFT KEY
		...
		DE 0x71; offset + 0x71  jamais utilise
		...
		DE 0x00; offset + 0x7F  jamais utilise
		DE 0x00; offset + 0x80  jamais utilise
		DE 0x00; offset + 0x81  jamais utilise
		DE 0x00; offset + 0x82  jamais utilise
		DE 0x00; offset + 0x83  jamais utilise
		DE 0x4A; offset + 0x84 KP . (POINT)
...
Tab_Temp
		...
		DE 180; valeur AN0 pour 0 degres     Eiffel 1.0.4
		DE 28; facteur d'echelle temperature Eiffel 1.0.4
;-----------------------------------------------------------------------------
